            ...
            'callback' => [
                ...
                "GetTimeCheck" => function ($model){
                    if($model->status > 1) {
                        ...
                        return $diff<=0?($model->status == 2?z_language('Đang kiểm tra'):z_language("Đợi đến lượt")):$days.z_language('Ngày').' '.$hours.z_language('giờ')." $minuts".z_language('phút')." $seconds".z_language('giây')." ";
                    }
                    return "Thành công";
                },
            ]
