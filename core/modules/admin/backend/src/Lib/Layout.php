...
class Layout{
    ...
    public static function row($option,$preview='',$row='',$layout = true){
        $html = '';
        $data_id = $layout?' data-id="'.md5(rand(2,100)).'"':'';
    ...
    public static function plugin($option,$layout = true){
        ...
        $label = isset($option['stg']['plugin'])?$option['stg']['plugin']:(isset($option['stg']['widget'])?$option['stg']['widget']:"");
        $html.='
                    <span class="remove btn btn-danger btn-xs"><i class="fa fa-times"></i></span>
                    <span class="configuration btn btn-info btn-xs"><i class="fa fa-gear"></i></span>
                    <span class="drag btn btn btn-primary btn-xs"><i class="fa fa-arrows"></i></span>
                ...
                <div class="view">
                    <div class="plugin" data-module="'.$option['stg']['module'].'" data-widget="'.$option['stg']['widget'].'" data-type="'.$option['stg']['type'].'" data-system="'.(isset($option['stg']['system'])?$option['stg']['system']:0).'">'.((isset($option['opt']['title']) && !empty($option['opt']['title']))?$option['opt']['title']:$option['stg']['module'].":".$label).'</div>
                </div>
            </div>
       ';
        return $html;
