...
class LayoutController extends \Zoe\Http\ControllerBackend
{
    ...
    {
        if (isset(app()->getConfig()['modules']['plugin.layout'][$type])) {
            $data_type = (app()->getConfig()['modules']['plugin.layout'][$type]);
            return $data_type;
        } else {
            $this->getCrumb();
            $arr = [];
            $lists = isset(app()->getConfig()['modules']['plugin.layout']) ? app()->getConfig()['modules']['plugin.layout'] : [];
    ...
    private function GetViewHelperBlade($conf)
    {
        if (isset($conf["layout"]["ViewHelper"]) && !empty($conf["layout"]["ViewHelper"]) && class_exists($conf["layout"]["ViewHelper"])) {
            $class = $conf["layout"]["ViewHelper"];
            return new $class();
        } else {
    ...
    private function GetGridBlade($conf)
    {
        if (isset($conf["layout"]["GirdBladeHelper"]) && !empty($conf["layout"]["GirdBladeHelper"]) && class_exists($conf["layout"]["GirdBladeHelper"])) {
            $class = $conf["layout"]["GirdBladeHelper"];
            return new $class();
        } else {
    ...
    public function ajaxFormConfig(Request $request)
    {
        ...
        $items = $request->all();
        $use = $this->getListType($items['type']);
...
//                $data['views']["temp"] = app()->getComponents()->template["template"];;
    ...
    {
        $items = $request->all();
        $use = $this->getListType($items['_type_group']);
        $obj_layout = new \Admin\Lib\LayoutBlade();
        if (isset($items["cfg"]['template'])) {
            if ($items['stg']['type'] == "components" || $items['stg']['type'] == "widgets") {
                $obj_layout->ViewHelper = $this->GetViewHelperBlade($use);
                $obj_layout->GridHelper = $this->GetGridBlade($use);
...
//        }
    ...
    {
        $this->sidebar('backend:plugin:layout:list');
        ...
        if(isset($data["_id"]) && !is_null($data["_id"])){
            ...
            if($model){
                ...
                if(isset($data["_ref"]) && !is_null($data["_ref"])){
                    ...
                    $url = route('backend:plugin:layout:list');
                }
            }else{
